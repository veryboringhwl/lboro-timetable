(() => {
	...
	const DomParser = {
		getSemesterInfo() {
			...
			if (!dropdown?.value.startsWith("sem"))
				throw new Error("Please select Semester 1 or 2.");
		...
		getDayIndex(row) {
			...
			return curr
				? Config.consts.days.indexOf(
						curr.querySelector(Config.selectors.weekday).textContent.trim(),
					)
				: -1;
		},
	};
		...
		const events = [...rows].flatMap((row) => {
			...
			return [...row.cells]
				...
				.flatMap((cell) => {
					...
					const cellDuration = Temporal.Duration.from({
					...
					const [roomRaw, buildRaw] = cell.querySelectorAll(
						Config.selectors.room,
					);
